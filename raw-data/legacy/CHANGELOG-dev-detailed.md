# CHANGELOG.md — 버전별 업데이트 기록 (개발자용 상세본, legacy)

> 이 파일은 과거 CHANGELOG.md를 그대로 옮겨온 개발자용 상세 기록입니다.
> 비전문가가 보기 쉬운 요약은 프로젝트 루트의 `CHANGELOG.md`를 참고하세요.
> 파일명/CSS 클래스/함수명 등 구현 세부사항을 포함한 원본 그대로이며,
> 앞으로는 이 파일을 갱신하지 않습니다.
