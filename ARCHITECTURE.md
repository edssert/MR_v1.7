...
└── raw-data/                      # 원본 근거 자료 아카이브 (사이트 동작과 무관)
    ├── raw-specs/                 #   제품 스펙 원문(docx/pdf) + 마크다운 정리본
    │   ├── README.md              #   보관 관례 설명
    │   └── <mk>/speakers|amplifiers/<series|type>/<model>.md
    ├── raw-images/                #   제품 사진 원본(가공 전) 아카이브
    │   ├── README.md
    │   └── <mk>/speakers/<series>/<원본 파일명>
    └── legacy/                    #   구 CHANGELOG 등 더 이상 갱신 안 하는 기록
        └── CHANGELOG-dev-detailed.md
```

> `raw-data/`는 실사이트에 쓰이는 `assets/img`(webp, 배경/뱃지 제거 등
> 가공 완료본)와 별개다. raw-specs·raw-images는 "원본이 무엇이었는지"
> 되짚어보기 위한 근거 자료 저장소일 뿐, 앱 실행에는 관여하지 않는다.
...
### 카드 배지: 모달용 vs 카드용 분리
`speakers.view.js` 에는 태그 배지를 만드는 함수가 두 개 있다: `titleTagsHTML`
(모달용, `d.crossoverTags` 원본 그대로 노출)과 `cardTagsHTML`(카드 그리드용,
축약본 — `(설명)` 괄호 제거, 단독 `Nch` 태그 제거, active 태그가 있으면
`passive` 중복 제거, active/passive 계열을 앞으로 정렬). 카드는 폭이 좁아 원본
태그를 그대로 쓰면 줄바꿈이 깨지므로 반드시 `cardTagsHTML` 을 쓴다. 새 배지
종류를 추가할 때 이 두 함수 중 어디에 넣을지 구분할 것.

### 정렬: schema 의 comparator ≠ 항상 최종 순서
카드 정렬은 기본적으로 `*.schema.js` 의 `sorters` comparator 로 결정되지만,
도메인 controller 가 `groupBy.sortWithinGroup` 을 별도로 지정해두면 그쪽이
우선한다(예: `amplifiers.controller.js`). 정렬 순서를 바꿀 때 schema 만 고치고
controller 를 놓치면 "고쳤는데 그대로"인 버그가 난다 — 두 곳을 항상 같이 확인.
고정 순서가 필요하면(알파벳순이 아닌 임의 순서) schema 에 `XXX_ORDER` 배열 +
`compareXxx()` 헬퍼를 만들고 sorters 와 controller 양쪽에서 재사용한다
(`amplifiers.schema.js` 의 `MODEL_ORDER`/`compareModel` 참고).

### 모달 이미지 확대(zoom) ↔ 뷰 전환 토글 CSS 레이어링
`.modal__media-wrap` 이 클리핑(`overflow: hidden`)을 전담하고, 그 안의
`.modal__media`(사진)는 클리핑하지 않는다 — 그래야 확대된 이미지가 토글 바
영역까지 자연스럽게 이어져 보인다. `.modal__view-switch`(토글 바)는
`position: relative; z-index: 1`로 항상 확대된 이미지 위에 뜨게 하고, 활성
버튼(`.is-active`)도 배경을 채우지 않아(색상/테두리로만 구분) 사진이 토글 바
아래까지 비쳐 보인다. 이 구조를 바꿀 때는 "클리핑=wrap, 배경=투명, z-index로
분리"라는 3원칙을 유지할 것.
