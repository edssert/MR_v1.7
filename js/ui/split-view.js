 ...
 */
import { getModalEl, wirePaneInteractions, closeModal, isMobileLayout, pushMobileModal, removeScrollbarTrack, setMediaLightboxOpener, setSplitViewCloser } from "./modal.js";
 ...
 */
function openMediaSplitPane(views, startIndex, paneColor, onViewChange) {
  ...
  const imgsHTML = views.map((v, i) =>
    `<img class="modal__img media-split-pane__img" data-view="${v.slug || `v${i}`}" src="${v.src}" alt="${v.alt}"${i === startIndex ? "" : " hidden"}>`
  ).join("");
