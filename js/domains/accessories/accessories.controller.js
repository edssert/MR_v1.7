...
function buildAccessoriesUI(wrap) {
      ...
      <div class="controls__inner">
        <div class="controls__row">
          <label class="search-box">
            ...
            <input class="search-box__input" id="acc-q" type="search" placeholder="액세서리 검색  ·  e.g.  Rigging / Cable / Case" autocomplete="off" spellcheck="false" />
          </label>
          <div class="controls__tools">
            <select class="select" id="acc-sort">
              <option value="type">정렬 · 유형별</option>
              <option value="name">정렬 · 이름순</option>
            </select>
            <button class="btn" id="acc-reset">필터 초기화</button>
            <button class="btn btn--toggle" id="acc-filter-toggle" type="button" aria-expanded="false">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
              필터
            </button>
          </div>
        </div>
        <div class="filter-panel" id="acc-filters"></div>
      </div>
    </div>
  ...
  $("#acc-q").addEventListener("input", e => { accState.q = e.target.value.trim(); renderAccessories(); });
...
export function initAccessoriesDomain() {
  registerDomain("accessories", { label: "Accessories", mount: mountAccessories, unmount: unmountAccessories, count: () => ACCESSORIES.length });
}
